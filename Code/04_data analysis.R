...
get_evaluation_metric <- function(index,sample_str){
  ...
  class_evaluation <- confusionMatrix(plasma_class,real_class)
  tibble(optimal_cutpoint = o$optimal_cutpoint, Accuracy = class_evaluation$overall[1], 
...
df_metric_all <- rbind(fit_cutpoint_Plasma_real,df_metric_100%>%col_summary(), df_metric_200%>%col_summary(),df_metric_500%>%col_summary())
